...
public:
	virtual bool On_Item_Add(class ALocalMap* _pLocalMap, FHHM_TileData& _tileData, class AEntity* _pEntity, TSharedPtr<UHHM_ItemData> _pItemData);
	virtual bool On_Item_Add_At(class ALocalMap* _pLocalMap, FHHM_TileData& _tileData, class AEntity* _pEntity, TSharedPtr<UHHM_ItemData> _pItemData, int32 _index_Inventory_Horizontal, int32 _index_Inventory_Vertical);
	//Remove 함수와 Delete 함수에 딱히 차이는 없음. 둘 다 이미 외부에서 지우고자 하는 아이템을 갖고있어, Remove_At과 달리 아이템을 받아오거나 하는 과정이 필요없기 때문.
	virtual bool On_Item_Remove(class ALocalMap* _pLocalMap, FHHM_TileData& _tileData, class AEntity* _pEntity, TSharedPtr<UHHM_ItemData> _pItemData_Remove);
	virtual bool On_Item_Remove_At(class ALocalMap* _pLocalMap, FHHM_TileData& _tileData, class AEntity* _pEntity, TSharedPtr<UHHM_ItemData>& _pItemData_Return, int32 _index_Inventory_Horizontal, int32 _index_Inventory_Vertical);
	virtual bool On_Item_Delete(class ALocalMap* _pLocalMap, FHHM_TileData& _tileData, class AEntity* _pEntity, TSharedPtr<UHHM_ItemData> _pItemData_Delete);
